...
代表脚本：`scripts/verify_er_layout_geometry.py`

原理：

1. 读取生成脚本输出的 `er_layout.json`。
2. 检查实体框、主题框、关系菱形、连接线段和 `1/n` 标记框的坐标。
3. 拦截连接线过短、关系线穿框、多条关系共用端口、关系线交叉、菱形压框和基数标记落入框内等问题。

适用场景：

- 总 E-R 图包含较多物理实体和逻辑主题，人工排版容易出现乱线时。
- 需要让不同 agent 在不同论文主题下复现“参考图式”的清爽布局时。

注意：

- 该脚本检查几何关系，不检查文字语义；实体是否来自源码仍由 `verify_project_content_completeness.py` 和人工证据台账负责。
- 脚本通过后仍必须导出 PDF/PNG 截图确认视觉效果，因为 Word 缩放后文字大小和图题同页情况仍需渲染验证。

### 2.7 代码图文字与字体检查脚本

代表脚本：`scripts/verify_code_figure_text.py`

原理：

1. 读取生成脚本输出的 `code_figures_manifest.json`，或直接解析生成脚本中的 `code_snippets` 字典。
2. 检查代码图源文本中是否已经出现 `????`、`�`、缺字方框等乱码标记。
3. 读取配置字体的 cmap，确认每个非 ASCII 字符至少被一个字体覆盖。

适用场景：

- 论文中使用自动生成的代码截图或 IDE 风格代码图。
- 代码片段包含中文字符串、中文注释或中文返回信息时。

注意：

- 英文等宽字体通常不能覆盖中文；代码图应使用等宽字体渲染 ASCII，并为中文字符配置 Noto Sans SC、微软雅黑或宋体等回退字体。
- 该脚本只能证明源文本和字体覆盖没有问题；最终仍需导出 PDF/PNG 视觉检查，确认 Word 缩放后没有缺字、过小或越框。
...
10. 执行黑白图形检查、项目内容完整性检查、E-R 图几何布局检查和代码图文字检查。
...
该测试会从示例生成脚本开始，依次执行封面复制、目录更新、OpenXML 后处理、前置页检查、右上角学号/档号检查、正文长度检查、目录检查、图表检查、`officecli validate` 和 `officecli view outline`。如果 `officecli validate` 输出 schema 错误，即使命令退出码为 0，也必须视为失败继续修复。
...
右上角学号/档号应执行 OpenXML 和渲染检查，至少确认：
...
正文长度必须单独按正文口径检查，不能用全文计数替代：

```text
BODY_TEXT_LENGTH_CHECK=PASS
BODY_NONSPACE_VISIBLE_CHARS >= 20000
```
